...
export const createBusiness = expressAsyncHandler(async (req, res) => {
...
export const updateBusiness = expressAsyncHandler(async (req, res) => {
    const serviceProvider = await ServiceProvider.findById(req.userId);
    if (!serviceProvider) {
        throw createHttpError(404, 'Service Provider not found');
    }
    ...
    const updatedBusiness = await Business.findByIdAndUpdate(serviceProvider.business, { $set: { ...req.body, serviceProvider: req.userId } }, { new: true, runValidators: true });
    res.status(200).json({
        success: true,
